...
if not MONGODB_URL:
    LOG.error(
        "Please use the enivironment variable MONGODB_URL to configure the database connection."
    )
mongo = MongoClient(
    host=MONGODB_URL,
    tz_aware=True,
...
try:
    ...
    LOG.exception(
        "Exception connecting to database. Shutting down this server using a kill signal"
    )
...
USER_LOOKUP_URL = os.getenv(
    "USER_LOOKUP_URL", "https://coact-dev-userlookup.slac.stanford.edu/graphql"
)
...
class CustomContext(BaseContext):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.userlookup = Client(
            transport=RequestsHTTPTransport(
                url=USER_LOOKUP_URL, verify=True, retries=3
            ),
    ...
    def isUserRegistered(self, **kwargs):
        ...
        if bool(environ.get("PREFER_EPPN", False)):
            ...
            if self.eppn:
                ...
                self.fullname = self.request.headers.get(
                    environ.get("FULLNAME_FIELD", "REMOTE_GECOS"), None
                )
    ...
    def authn(self, **kwargs):
        ...
        if self.origin_username:
            ...
            if self.origin_username in ADMINS:
                ...
                if (
                    "coactimp" in self.request.headers
                    and self.request.headers["coactimp"]
                    and self.request.headers["coactimp"] != "null"
                ):
                    user = self.db.find_user(
                        {"username": self.request.headers["coactimp"]}
                    )
                    self.LOG.warning(
                        f"user {self.username} is impersonating {user.username}"
                    )
                ...
                if (
                    ...
                    raise Exception(
                        f"unauthorised attempt by user {self.username} to impersonate {kwargs['impersonate']}"
                    )
            self.showallforczars = json.loads(
                self.request.headers.get("coactshowall", "false")
            )
            if self.showallforczars:
                facilities = self.db.find_facilities({"czars": self.username})
                if not facilities and not self.is_admin:
                    LOG.warning(
                        f"Showall is set for user {self.username} who is not an admin or czar. Ignoring"
                    )
    ...
    def lookupUserInService(self, preferredUserName):
        resp = self.userlookup.execute(
            lookupUser, variable_values={"filter": {"username": preferredUserName}}
        )
        if resp["users"]:
            return resp["users"][0]
        return None
    ...
    def lookupUserInServiceUsingEPPN(self, eppn):
        resp = self.userlookup.execute(
            lookupUser, variable_values={"filter": {"eppns": eppn}}
        )
        if resp["users"]:
            return resp["users"][0]
        return None
    ...
    def notify(self, request: CoactRequest) -> bool:
        ...
        try:
            if request.preferredUserName:
                ...
                if luobj:
                    ...
                    LOG.info(
                        f"Userlookup service returned {preferredemail} for {request.preferredUserName}"
                    )
                    user_email = [
                        preferredemail,
                    ]
                ...
                LOG.error(
                    f"Userlookup service does not have an entry for {request.preferredUserName}. Sending email to the EPPN instead {request.eppn}"
                )
                user_email = [
                    request.eppn,
                ]
        ...
        LOG.info(
            f">>> TEMPLATE: {template_prefix}, FACILITY: {facility}, CZARS: {czars}, CZAR EMAIL: {czar_emails}, USER: {user}, EMAIL: {user_email}, DATA: {self.db.to_dict(request)}"
        )
        return self.email.notify(
            request_type=request_type,
            request_status=request_status,
            data=self.db.to_dict(request),
            template_prefix=template_prefix,
            user=user_email,
            czars=czar_emails,
            skip_czar_emails=skip_czar_emails,
        )
    ...
    def dict_diffs(self, prev, curr):
        ...
        fwd_changes = dict(
            set(__expand_dict__(curr_dict)) - set(__expand_dict__(prev_dict))
        )
        bwd_changes = dict(
            set(__expand_dict__(prev_dict)) - set(__expand_dict__(curr_dict))
        )
        ...
        all_changes = [
            str(k) + ": " + str(bwd_changes[k]) + " -> " + str(fwd_changes[k])
            for k in changed
        ]
        all_changes.extend([str(k) + ": N/A -> " + str(fwd_changes[k]) for k in added])
        all_changes.extend(
            [str(k) + ": " + str(bwd_changes[k]) + " -> N/A" for k in removed]
        )
        return "\n".join(all_changes)

    def lookupUsersFromService(self, filter):
        try:
            users = self.userlookup.execute(
                lookupUser, variable_values={"filter": self.db.to_dict(filter)}
            )["users"]
...
class DB:
    ...
    def find(self, thing: str, filter, exclude_fields=[]):
        ...
        self.LOG.debug(
            f"searching for {thing} using {filter} -> {search} (excluding fields {exclude_fields})"
        )
    ...
    def assert_one(self, items, filter):
        if len(items) == 0:
            raise AssertionError(
                f"did not find any matching items using filter {filter}"
            )
        elif len(items) > 1:
            raise AssertionError(
                f"found too many items using filter {filter}, only expecting one"
            )
        return items[0]
    ...
    def czars(self, facilityname: str) -> List[str]:
        f = self.collection("facilities").find_one(
            {"name": facilityname}, {"_id": 0, "czars": 1}
        )
        return f["czars"]
...
class Email:
    ...
    def __init__(
        ...
        server,
        port,
        fm="no-reply-s3df-help@slac.stanford.edu",
        subject_prefix="[Coact] ",
        assets_path="./assets/notifications/email/",
    ):
    ...
    def notify(
        ...
        user: str,
        czars: List[str] = [],
        skip_czar_emails=False,
        admins: List[str] = [
            "sdf-help@slac.stanford.edu",
        ],
        dry_run: bool = False,
    ) -> bool:
        ...
        templates = [
            ...
            if f.startswith(template_prefix) and f.endswith(self.template_extension)
        ]
...
schema = Schema(
    query=Query,
    mutation=Mutation,
    scalar_overrides={datetime: CoactDatetime},
...
if REQUEST_STREAM:
    ...
    graphql_service_app = GraphQLRouter(service_schema, context_getter=get_context)
...
DEV_MODE = environ.get("DEV_MODE", "false").lower() == "true"
if DEV_MODE:
    print("🚨 DEV_MODE is enabled")
    DEV_USERNAME = environ.get("DEV_USERNAME", "testuser")
    DEV_EPPN = environ.get("DEV_EPPN", "testuser@slac.stanford.edu")
    DEV_FULLNAME = environ.get("DEV_FULLNAME", "Test User")

    LOG.warning(
        f"🚨 DEV_MODE enabled - injecting auth headers for user: {DEV_USERNAME}"
    )

    @app.middleware("http")
    async def inject_dev_headers(request: Request, call_next):
        # Create a new scope with injected headers
        request.scope["headers"] = [
            ...
            not in [
                b"x-vouch-user",
                b"x-vouch-idp-claims-name",
                b"x-vouch-idp-claims-given_name",
            ]
        ]
        # Add dev headers
        request.scope["headers"].extend(
            [
                (b"x-vouch-user", DEV_EPPN.encode()),
                (b"x-vouch-idp-claims-name", DEV_USERNAME.encode()),
                (b"x-vouch-idp-claims-given_name", DEV_FULLNAME.encode()),
            ]
        )
...
if REQUEST_STREAM:
    GRAPHQL_SERVICE_PREFIX = environ.get(
        "COACT_GRAPHQL_SERVICE_PREFIX", "/graphql-service"
    )
